...
layout(std430, binding = 3) buffer Accumulation{
  vec4 accumulation[];
};
...
void main(){
  ...
  int rngState = int(int(gl_FragCoord.x) * int(1973) + int(gl_FragCoord.y) * int(9277) + int(iFrame) * int(26699)) | int(1);
  vec2 uv = (gl_FragCoord.xy / resolution) * 2.0 - 1.0;

  vec4 rayClip = vec4(uv, -1.0, 1.0);

  // clip → view
  vec4 rayView = projMat * rayClip;
  rayView /= rayView.w;

  // view → world (direction)
  vec3 rayDir = normalize((viewMat * vec4(rayView.xyz, 0.0)).xyz);
  ...
  vec3 pixelColor = totalIncomingLight / NUM_RAYS_PER_PIXEL;
